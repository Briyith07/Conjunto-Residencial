...
def generar_pdf(titulo, datos, columnas, filename, usuario):
   ...
   # Configuración de la tabla
    col_widths = [60, 100, 250, 100, 150]  # Ancho personalizado por columna
    ...
    if table_width > width - 100:  # Si la tabla es más ancha que la página
        # Ajustar el ancho de las columnas proporcionalmente para que quepan
        col_widths = [(width - 100) / len(columnas)] * len(columnas)
        table = Table(data, colWidths=col_widths)
        table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.grey),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
...
# Calcular la altura total de la tabla para verificar si cabe en la página
    table_height = len(datos) * 20
    if y_position - table_height < 50:  # Si la tabla no cabe en la página
        c.showPage()  # Crear nueva página
        y_position = height - 50  # Reiniciar la posición Y
...
def generar_reporte_reservas(filtro, id_rol, id_torre, id_apartamento, fecha_inicio, fecha_fin, tipo_exportacion):
    ...
    if tipo_exportacion == 'excel':
        ...
        filepath = generar_excel(titulo, datos, columnas, filename)
        return send_file(filepath, as_attachment=True)
    else:
